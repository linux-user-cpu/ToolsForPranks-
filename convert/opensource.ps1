...
$user_input = Read-Host "Do you accept these terms, freely and without reservation, entering into this agreement with full understanding of its bearing? Type 'YES' or 'NO'"

if ($user_input -eq "YES") {
    Write-Host "Welcome, member."
} else {
    Write-Host ""
}

if ($user_input -eq "NO") {
    Write-Host "Alas, the offer stands rebuffed. May your code remain as it was, guarded and unshared, though the door to open source shall always remain ajar, should you reconsider."
} else {
    Write-Host ""
}
